...
class Napoli_Pro_Author_Bio {
	...
	static function setup() {
		...
		add_action( 'napoli_author_bio', array( __CLASS__, 'author_bio' ) );
		...
		add_filter( 'napoli_hide_elements', array( __CLASS__, 'hide_author_bio' ) );
	}
		...
		if ( true === $theme_options['author_bio'] || is_customize_preview() ) : ?>
			...
			<div class="entry-author clearfix">
				...
				<div class="author-heading">
					<h4 class="author-title"><?php printf( esc_html__( 'About %s', 'napoli-pro' ), '<span class="author-name">' . get_the_author() . '</span>' ); ?></h4>
					<a class="author-link" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
						<?php esc_html_e( 'View all posts', 'napoli-pro' ); ?>
					</a>
				</div>
	...
	static function author_bio_settings( $wp_customize ) {
		...
		$wp_customize->add_setting( 'napoli_theme_options[author_bio]', array(
			'default'           => false,
			'type'              => 'option',
		...
		$wp_customize->add_control( 'napoli_theme_options[author_bio]', array(
			'label'    => __( 'Display Author Bio', 'napoli-pro' ),
			'section'  => 'napoli_section_post',
			'settings' => 'napoli_theme_options[author_bio]',
			'type'     => 'checkbox',
			'priority' => 85,
		) );
	}
...
// Run Class.
add_action( 'init', array( 'Napoli_Pro_Author_Bio', 'setup' ) );
